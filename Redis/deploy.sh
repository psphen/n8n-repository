#!/bin/bash
# deploy.sh - Despliega Redis en n8n-redis (10.114.158.75)
# Ejecutar desde WSL, Git Bash o Linux con sshpass instalado
# Instalación sshpass: sudo apt-get install sshpass  (Linux/WSL)

set -e

SERVER_IP="10.114.158.75"
SERVER_USER="administrador"
SERVER_PASS="Temporal01#"
REMOTE_DIR="/opt/n8n-redis"

echo "=== Desplegando Redis en $SERVER_IP ==="

# Verificar sshpass
if ! command -v sshpass &> /dev/null; then
    echo "ERROR: sshpass no encontrado. Instalar con: sudo apt-get install sshpass"
    exit 1
fi

SSH="sshpass -p '$SERVER_PASS' ssh -o StrictHostKeyChecking=no $SERVER_USER@$SERVER_IP"
SCP="sshpass -p '$SERVER_PASS' scp -o StrictHostKeyChecking=no"

# 1. Instalar Docker si no está presente
echo "[1/5] Verificando Docker..."
sshpass -p "$SERVER_PASS" ssh -o StrictHostKeyChecking=no "$SERVER_USER@$SERVER_IP" "
    if ! command -v docker &> /dev/null; then
        echo 'Instalando Docker...'
        curl -fsSL https://get.docker.com | sudo sh
        sudo systemctl enable docker
        sudo systemctl start docker
        sudo usermod -aG docker $USER
    else
        echo 'Docker ya está instalado.'
    fi
"

# 2. Crear directorio remoto
echo "[2/5] Creando directorio $REMOTE_DIR..."
sshpass -p "$SERVER_PASS" ssh -o StrictHostKeyChecking=no "$SERVER_USER@$SERVER_IP" \
    "sudo mkdir -p $REMOTE_DIR && sudo chown $SERVER_USER:$SERVER_USER $REMOTE_DIR"

# 3. Copiar docker-compose.yml
echo "[3/5] Copiando docker-compose.yml..."
sshpass -p "$SERVER_PASS" scp -o StrictHostKeyChecking=no \
    docker-compose.yml "$SERVER_USER@$SERVER_IP:$REMOTE_DIR/docker-compose.yml"

# 4. Detener servicio existente
echo "[4/5] Deteniendo redis-server existente..."
sshpass -p "$SERVER_PASS" ssh -o StrictHostKeyChecking=no "$SERVER_USER@$SERVER_IP" "
    sudo systemctl stop redis-server 2>/dev/null || true
    sudo systemctl disable redis-server 2>/dev/null || true
    echo 'Servicio redis-server detenido.'
"

# 5. Levantar contenedor Docker
echo "[5/5] Levantando contenedor Redis..."
sshpass -p "$SERVER_PASS" ssh -o StrictHostKeyChecking=no "$SERVER_USER@$SERVER_IP" "
    cd $REMOTE_DIR
    sudo docker compose pull
    sudo docker compose up -d
    echo ''
    echo '--- Estado del contenedor ---'
    sudo docker compose ps
    echo ''
    echo '--- Test de conectividad ---'
    sleep 3
    sudo docker exec n8n-redis redis-cli ping
"

echo ""
echo "=== Redis desplegado exitosamente en $SERVER_IP:6379 ==="
