#!/bin/bash
# deploy.sh - Despliega n8n Main 2 en n8n-nodo2 (10.114.158.72)
# NOTA: Main 2 es backup. Desplegar después de que Main 1 esté operativo.
# Ejecutar desde WSL, Git Bash o Linux con sshpass instalado

set -e

SERVER_IP="10.114.158.72"
SERVER_USER="administrador"
SERVER_PASS="Temporal01#"
REMOTE_DIR="/opt/n8n-main2"

echo "=== Desplegando n8n Main 2 (backup) en $SERVER_IP ==="
...
# 4. Detener servicio n8n existente
echo "[4/5] Deteniendo n8n systemd existente..."
sshpass -p "$SERVER_PASS" ssh -o StrictHostKeyChecking=no "$SERVER_USER@$SERVER_IP" "
    sudo systemctl stop n8n 2>/dev/null || true
    sudo systemctl disable n8n 2>/dev/null || true
    echo 'Servicio n8n detenido.'
"

# 5. Levantar contenedor Docker
echo "[5/5] Levantando contenedor n8n Main 2..."
sshpass -p "$SERVER_PASS" ssh -o StrictHostKeyChecking=no "$SERVER_USER@$SERVER_IP" "
    cd $REMOTE_DIR
...
echo ""
echo "=== n8n Main 2 (backup) desplegado exitosamente ==="
echo "    Puerto local: http://$SERVER_IP:5678"
